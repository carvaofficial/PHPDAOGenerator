...
/**
 * Main function
 * First select all tables from de DB, then check the selected databases and finally generates their classes 
 */
function generate()
{
	...
	$sql = "SHOW TABLES";
	$all_db_tables = QueryExecutor::execute(new SqlQuery($sql));

	$final_tables = array();
	$selected_tables = $_POST['table'];

	for ($i = 0; $i < count($all_db_tables); $i++) {
		if (in_array($all_db_tables[$i][0], $selected_tables)) {
			$final_tables[] = $all_db_tables[$i];
		}
	}

	generateDomainObjects($final_tables);
	generateDAOObjects($final_tables);
	generateDAOExtObjects($final_tables);
	generateIDAOObjects($final_tables);
	if (!$CI_compatible) {
		createIncludeFile($final_tables);
	}
	createDAOFactory($final_tables, $CI_compatible);

	echo "<script>alert('Generation complete');</script>";
}

/**
 * Overwrites the last DAO generation (if exists)
 */
...
/**
 * Creates all the dirs for DAO classes generation
 */
...
function createDAOFactory($tables, $CI_compatible)
{
	$str = "\n";
...
function generateDomainObjects($tables)
{
	for ($i = 0; $i < count($tables); $i++) {
		if (!doesTableContainPK($tables[$i])) {
			continue;
