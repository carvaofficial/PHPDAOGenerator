...
<form method="post" action="generate.php">
    ...
    <fieldset>
        ...
        <p>Select all the tables you want to generate code <button onclick="selectAll(event);">Select all</button></p>

        <?php
        for ($i = 0; $i < count($tables); $i++) {
            echo '<label><input name="table[]" type="checkbox" value="' . $tables[$i][0] . '"/>' . $tables[$i][0] . '</label><br/>';
        }
        ?>
    </fieldset>
    ...
    <fieldset>
        ...
        <label><input type="radio" name="type" checked="checked" value="0">Vanilla PHP</label>
        <br>
    ...
    <p><input type="submit" name="generate" id="generate" value="Generate" /></p>
</form>
